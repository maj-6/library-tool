(function installCorrectionsShell(root, factory) {
  ...
  class CorrectionsShell {
    ...
    handleQueuedTransformResult(result, command, _resource, observation = null) {
      ...
      const extraction = Boolean(command && isPlainObject(command.extraction));
      ...
      if (!imageProcessing && !extraction) this.advanceAfterQueuedPerspective(command);
      if (!extraction) this.reportTransformFailure(result, command, observation);
    ...
    connectTransformResults() {
      ...
      try {
        const release = this.subscribeTransformResults((result, command = null) => {
          ...
          this.captureExtractionTerminal(result, command, observation);
          if (!(command && isPlainObject(command.extraction))) {
