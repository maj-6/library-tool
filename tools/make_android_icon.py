"""Regenerate Book Capture's adaptive-icon foregrounds from its artwork master.
...
``ic_launcher_safe_fg.xml`` insets this bitmap by 14.5 dp on all four edges, so
the 108 dp bitmap is drawn into a 79 dp box. Working backwards, the artwork's
*diagonal* must satisfy::

    diagonal_in_bitmap * (79 / 108) <= 72 dp

which caps the diagonal at 91.1% of the bitmap width. ``SAFE_DIAGONAL`` sits a
little under that so rounding at mdpi can't nudge a corner over the line. The
14.5 dp inset is asserted by ``ResourceContractTest`` — change it there first if
it ever has to move.
"""
...
# 72/108 * 108/79 = 0.9114; 0.867 leaves ~3.5 dp of headroom inside the mask and
# matches the framing the mark shipped with before.
SAFE_DIAGONAL = 0.867
